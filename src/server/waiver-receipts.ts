...
const participantLabel = (participant: WaiverReceiptEnvelope["acceptance"]["participants"][number]) => {
  if (participant.participationBasis === "minor_guardian_permission") {
    return `${participant.fullName} (minor account holder; guardian permission recorded)`;
  }
  return participant.role === "minor"
    ? `${participant.fullName} (birth year ${participant.birthYear})`
    : `${participant.fullName} (adult account holder)`;
};

const participantText = (envelope: WaiverReceiptEnvelope) =>
  envelope.acceptance.participants.map(participantLabel);
...
export const renderWaiverReceipt = (
  ...
  const hasSupervisedMinors = acceptance.participants.some(
    (participant) => participant.role === "minor" && participant.participationBasis === undefined,
  );
  const statements = [
    generatedParticipationWaiver.acceptanceStatement,
    ...(hasSupervisedMinors ? [generatedParticipationWaiver.guardianStatement] : []),
  ];
