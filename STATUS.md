...
<https://b8d2dc2f.seba-treasure-hunt.pages.dev/>

Deployment ID: `b8d2dc2f-7971-44f8-a049-400bdf0003bd`. Deployed: 2026-07-13 from source `942ec9e`. This pass uploaded `dist/` only to Pages project `seba-treasure-hunt`, branch `codex-validation`. It did not apply a D1 migration, set a secret, change Turnstile, deploy a media worker, change DNS/custom domains, submit an inquiry, or touch production.
...
- The current noindex preview and private validation media consumer are deployed successfully. The July 13 sponsor Pages preview is deployment `b8d2dc2f-7971-44f8-a049-400bdf0003bd`, source `942ec9e`; production remains deployment `ad89ff2a-5818-4546-ba8f-3f1b7cd25359`, source `5552a57`, branch `main`.
...
- Sponsor implementation commits through `2f8d157` are locally verified and included in the validation deployment built from `942ec9e`. Focused API, authorization, D1 store/integration, client behavior, static privacy, and legal-hash tests pass.
...
- Live sponsor validation smoke: both `https://codex-validation.seba-treasure-hunt.pages.dev` and immutable deployment `https://b8d2dc2f.seba-treasure-hunt.pages.dev` serve `/sponsors` with HTTP 200, `X-Robots-Tag: noindex, nofollow`, the persistent validation notice, and the sponsor form. The home page exposes Sponsors in its persistent navigation and footer. With the public keys absent, the human check visibly reports unavailable and submit remains disabled; no POST was attempted.
