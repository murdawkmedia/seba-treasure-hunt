...
- Migration `0005_sponsor_inquiries.sql` is exercised against local Miniflare D1, including atomic events, literal search, tuple pagination, state totals, and concurrent transitions. The validation migration approach remains an explicit decision: either replay the idempotent 0001-through-0005 files so Wrangler records them, or first audit exact live DDL before any surgical migration-ledger reconciliation.
