...
test("the case-room console exposes every approved ledger and safe account control", () => {
  ...
  assert.match(client, /\/api\/v1\/ops\/updates\/\$\{encodeURIComponent/);
  assert.match(client, /Draft saved privately\. Nothing was published\./);
  assert.match(client, /Images uploaded privately\. Select the images to include\./);
