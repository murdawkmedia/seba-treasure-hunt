...
import { isAcceptedAuthorizedPartyClaim, isAllowedAuthorizedParty } from "../src/server/auth";
...
test("accepts Clerk's documented omitted azp claim but rejects malformed or hostile claims", () => {
  const configured = "https://codex-validation.seba-treasure-hunt.pages.dev";
  assert.equal(isAcceptedAuthorizedPartyClaim(undefined, configured), true);
  assert.equal(isAcceptedAuthorizedPartyClaim(null, configured), false);
  assert.equal(isAcceptedAuthorizedPartyClaim("https://attacker.test", configured), false);
  assert.equal(
    isAcceptedAuthorizedPartyClaim("https://codex-validation.seba-treasure-hunt.pages.dev", configured),
    true
  );
});
