...
test("waiver receipt identifies a minor account holder without inventing a birth year", () => {
  const minorEnvelope: WaiverReceiptEnvelope = {
    ...envelope,
    acceptance: {
      ...envelope.acceptance,
      participants: [{
  ...
  const message = renderWaiverReceipt(minorEnvelope, "https://www.timlostsomething.com");
  assert.match(message.text, /Young Hunter \(minor account holder; guardian permission recorded\)/);
  assert.match(message.html, /minor account holder; guardian permission recorded/);
  assert.doesNotMatch(message.text, /birth year null/);
  assert.doesNotMatch(message.text, new RegExp(participationWaiverDocument.guardianStatement.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
