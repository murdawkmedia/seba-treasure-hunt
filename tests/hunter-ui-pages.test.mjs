...
test("report form is accessible, human-checked, and keeps geolocation optional", async () => {
  ...
  assert.match(client, /sent privately to the SebaHub case team/i);
  assert.doesNotMatch(client, /It will remain private/i);
  assert.match(html, /data-report-receipt[^>]*hidden/);
  assert.match(html, /id="report-receipt-title">We got it<\/h2>/);
...
test("static community and reporting fallbacks preserve stable waypoint ids in public order", () => {
  for (const [file, selectId] of [
    ...
    assert.match(select, /Stop 11 · Driving Range \/ Brewing at Seba/);
  }
});
...
test("What People Found and finder reports explain moderated sharing before submission", () => {
  ...
  assert.match(report, /Share after staff review/i);
  assert.match(report, /Keep this report private/i);
