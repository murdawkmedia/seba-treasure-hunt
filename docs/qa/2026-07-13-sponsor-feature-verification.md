...
- Validation notice fixture at 1440×1000: uses the server's existing notice when present and injects the same validation notice markup at the start of the real page body only when the target lacks it. It requires exactly one notice, the shipped `position: relative` CSS, measured initial strip/header flow below the notice, measured sticky rows after deterministic scroll, and zero sponsor POSTs.
